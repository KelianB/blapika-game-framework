...
class TileMap {
   ...
   applyScalingX(value) {return Engine.utils.bitwiseRound(this.getScalingX() * value);};
   applyScalingY(value) {return Engine.utils.bitwiseRound(this.getScalingY() * value);};
   ...
   */
   render(ctx) {
      ...
      ctx.save();
         ...
         ctx.translate(Engine.utils.bitwiseRound(this.getTranslationX() * Engine.viewport.renderScaling.x),
                       Engine.utils.bitwiseRound(this.getTranslationY() * Engine.viewport.renderScaling.y));
   ...
   */
   ...
   canvasPosToMapPos(pos) {
      let cameraScaling = this.camera ? this.camera.scaling : 1;
      return {
         x: (pos.x - this.getTranslationX()) / cameraScaling,
         y: (pos.y - this.getTranslationY()) / cameraScaling
      }
   };
